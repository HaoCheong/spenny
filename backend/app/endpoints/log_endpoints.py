...
@router.post("/api/v1/log", response_model=schemas.LogReadNR, tags=['Log'])
def create_log(log: schemas.LogCreate, db: Session = Depends(get_db)):
    return cruds.create_log(db=db, log=log)


@router.get("/api/v1/logs", response_model=List[schemas.LogReadNR], tags=['Log'])
def get_all_logs(limit: int = 1000, db: Session = Depends(get_db)):
    return cruds.get_all_logs(db=db, limit=limit)


@router.get("/api/v1/logs/{bucket_id}/{date}", response_model=List[schemas.LogReadNR], tags=['Log'])
...
@router.get('/api/v1/log/{log_id}', response_model=schemas.LogReadWR, tags=['Log'])
...
@router.patch('/api/v1/log/{log_id}', response_model=schemas.LogReadWR, tags=['Log'])
...
@router.delete('/api/v1/log/{log_id}', tags=['Log'])
